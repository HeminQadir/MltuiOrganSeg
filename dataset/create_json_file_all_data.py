...
def create_json(root_directory, k_folds=5):
    ...
    # Listing all the datasets in the root directory 
    folders = list_folders(root_directory)
    print(folders)

    for folder in folders: 

        subfolders = list_folders(os.path.join(root_directory, folder))

        print(subfolders)

        for subfolder in subfolders:
            # Get a list of files in "imagesTs" and "labelsTr"
            images_directory = os.path.join(root_directory, folder, subfolder, 'imagesTr')    
            labels_directory = os.path.join(root_directory, folder, subfolder, 'labelsTr')
            ...
            for train_index in sublists:
                ...
                fold_files_labels = [files_label.replace('imagesTr', 'labelsTr') for files_label in train_index]

                print(fold_files_labels)
...
if __name__ == "__main__":
    root_directory = "/media/samsung_ssd_1/Medical_Dataset"  # Replace with the actual path to your root directory
